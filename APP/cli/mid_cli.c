...
#define CLI_VERSION_MAJOR		(0)		/* major version  number */
#define CLI_VERSION_MINOR		(1)		/* minor version  number */
...
BaseType_t mid_cli_register(const struct _command_t * const p)
{
	...
	if( pxNewListItem != NULL )
	{
		taskENTER_CRITICAL();
		{
			...
			/* set end-flag as NULL for the list  */
			pxNewListItem->next = NULL;

			/* link new member */
			pxLastCommandInList->next = pxNewListItem;

			/* record cuttent node and waiting for the next node to be linked */
...
static void mid_cli_console_task(void *parame)
{
	...
	for(;;)
	{
		...
		if(status == 0)
		{
			/* waiting for new character come */
			...
			{
				#ifdef CLI_SUPPORT_PASSWD
				status = 1; /* certification authority */
				#else
				status = 3; /* prase the command instead of certification authority */
				#endif
				hal_cli_data_tx((char *)pcNewLine, strlen(pcNewLine));
			}
			else			/* is packing,support function of backspace to delete the characters */
			{
		...
		#ifdef CLI_SUPPORT_PASSWD
		/* packing the message is done and then  */
		if(status == 1)	
		{
			if(permission == PASSWD_INCORRECT)
				status = 2;
	...
	if(cmd == NULL)
	{
		for(cmd = &cmd_list_head; cmd != NULL; cmd = cmd->next)
		{
			...
			if((input[cmdStringLen] == cmdASCII_SPACE) || (input[cmdStringLen] == 0))
			{
				...
				{
					/* 
					 * we considered that the command is invalid
					 * while the number of input parameters unconformity
					 */
